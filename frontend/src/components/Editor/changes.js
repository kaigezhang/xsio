...
export function splitAndInsertBlock(change, options) {
  console.log(options, 'options props');
  ...
  change
    ...
    .setBlock(type, { normalize: false });

  if (wrapper) { change.wrapBlock(wrapper); }
  return change;
}

export async function insertImageFile(change, file, editor, onImageUploadStart, onImageUploadStop) {
  console.log(file, editor, onImageUploadStart, onImageUploadStop, 'changes props');
  onImageUploadStart();
  ...
  try {
    ...
    reader.addEventListener('load', () => {
      ...
      // insert / replace into document as uploading placeholder replacing empty
    ...
    const placeholder = editor
      .value
      .document
      .findDescendant(node => node.data && node.data.get('id') === id);
